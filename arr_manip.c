...
long arrayManipulation(int n, int queries_rows, int queries_columns, int** queries) 
{
    ...
    {
        arr[queries[i][0]] += queries[i][queries_columns-1];
        if((queries[i][queries_columns-2]+1) <= n)
            arr[queries[i][queries_columns-2]+1] -= queries[i][queries_columns-1];
    }
